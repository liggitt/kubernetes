...
func init() {
	fixtureData_1_0 := fixtureGenerator{
		expectErrorSubstring: "forbidden proc mount type",
		generatePass: func(p *v1.Pod) []*v1.Pod {
			p = ensureSecurityContext(p)
			return []*corev1.Pod{
				// set proc mount of container to a valid value
				...
				// don't set proc mount type
				tweak(p, func(copy *v1.Pod) {}),
				// set proc mount of init container to a valid value
				tweak(p, func(copy *v1.Pod) {
					validProcMountType := v1.DefaultProcMount
