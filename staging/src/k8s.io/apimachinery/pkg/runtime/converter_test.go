...
"unknown2": "foo"
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			err = runtime.NewTestUnstructuredConverterWithValidation(simpleEquality).FromUnstructuredWithValidation(unstr, tc.obj, tc.validationDirective)
...
func BenchmarkFromUnstructuredWithValidation(b *testing.B) {
	...
	for _, directive := range []string{"Ignore", "Warn", "Strict"} {
		b.Run(directive, func(b *testing.B) {
			newObj := reflect.New(reflect.TypeOf(iObj).Elem()).Interface()
			b.ReportAllocs()
			for i := 0; i < b.N; i++ {
				if err = runtime.NewTestUnstructuredConverterWithValidation(simpleEquality).FromUnstructuredWithValidation(unstr, newObj, directive); err != nil {
