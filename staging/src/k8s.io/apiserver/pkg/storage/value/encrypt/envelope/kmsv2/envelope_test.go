...
func TestEnvelopeCaching(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.desc, func(t *testing.T) {
			...
			envelopeTransformer, err := NewEnvelopeTransformer(envelopeService, tt.cacheSize, aestransformer.NewGCMTransformer, "testplugin")
			if err != nil {
...
func TestEnvelopeCacheLimit(t *testing.T) {
	envelopeTransformer, err := NewEnvelopeTransformer(newTestEnvelopeService(), testEnvelopeCacheSize, aestransformer.NewGCMTransformer, "testplugin")
	if err != nil {
		t.Fatalf("failed to initialize envelope transformer: %v", err)
	}
	ctx := context.Background()
...
func TestDecodeError(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedError: fmt.Errorf("pluginName mismatch, expected %s, got testplugin0", et.pluginName),
		},
	}
