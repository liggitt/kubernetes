...
func newETCD3Storage(c storagebackend.ConfigForResource, newFunc, newListFunc func() runtime.Object, resourcePrefix string) (storage.Interface, DestroyFunc, error) {
	...
	store := etcd3.New(client, compactor, c.Codec, newFunc, newListFunc, c.Prefix, resourcePrefix, c.TrackObjectSizeStats, c.GroupResource, transformer, c.LeaseManagerConfig, decoder, versioner)
	var once sync.Once
	destroyFunc := func() {
