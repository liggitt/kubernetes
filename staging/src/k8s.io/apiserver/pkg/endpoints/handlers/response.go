...
func transformObject(ctx context.Context, obj runtime.Object, opts interface{}, mediaType negotiation.MediaTypeOptions, scope RequestScope, req *http.Request) (runtime.Object, error) {
	...
	case target.Kind == "Table" && target.GroupVersion() == metav1beta1.SchemeGroupVersion:
		// TODO: Table does not support protobuf at the current time due to limitations in our generation
		// code (table cells are []interface{} and our generators have issues with type aliasing to create
		// an appropriate serializer).
		if accepted := mediaType.Accepted; accepted == nil || accepted.Type != "application" || accepted.SubType != "json" {
			return nil, negotiation.NewNotAcceptableConversionError(target.String(), []string{"application/json"})
		}
