...
func TestRecordAuthorizationConfigAutomaticReloadFailure(t *testing.T) {
	expectedValue := `
	# HELP apiserver_authorization_config_controller_automatic_reload_failures_total [ALPHA] Total number of failed automatic reloads of authorization configuration split by apiserver identity.
    # TYPE apiserver_authorization_config_controller_automatic_reload_failures_total counter
    apiserver_authorization_config_controller_automatic_reload_failures_total {apiserver_id_hash="sha256:14f9d63e669337ac6bfda2e2162915ee6a6067743eddd4e5c374b572f951ff37"} 1
	`
	metrics := []string{
		namespace + "_" + subsystem + "_automatic_reload_failures_total",
	}

	authorizationConfigAutomaticReloadFailureTotal.Reset()
	RegisterMetrics()
...
func TestRecordAuthorizationConfigAutomaticReloadSuccess(t *testing.T) {
	expectedValue := `
	# HELP apiserver_authorization_config_controller_automatic_reload_success_total [ALPHA] Total number of successful automatic reloads of authorization configuration split by apiserver identity.
    # TYPE apiserver_authorization_config_controller_automatic_reload_success_total counter
    apiserver_authorization_config_controller_automatic_reload_success_total {apiserver_id_hash="sha256:14f9d63e669337ac6bfda2e2162915ee6a6067743eddd4e5c374b572f951ff37"} 1
	`
	metrics := []string{
		namespace + "_" + subsystem + "_automatic_reload_success_total",
	}

	authorizationConfigAutomaticReloadSuccessTotal.Reset()
	RegisterMetrics()
...
func TestAuthorizationConfigAutomaticReloadLastTimestampSeconds(t *testing.T) {
	testCases := []struct {
		...
		{
			expectedValue: `
                # HELP apiserver_authorization_config_controller_automatic_reload_last_timestamp_seconds [ALPHA] Timestamp of the last successful or failed automatic reload of authorization configuration split by apiserver identity.
