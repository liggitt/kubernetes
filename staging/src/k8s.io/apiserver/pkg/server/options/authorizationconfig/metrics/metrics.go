...
var (
	authorizationConfigAutomaticReloadsTotal = metrics.NewCounterVec(
		&metrics.CounterOpts{
			...
			Help:           "Total number of failed automatic reloads of authorization configuration split by status and apiserver identity.",
	...
	authorizationConfigAutomaticReloadLastTimestampSeconds = metrics.NewGaugeVec(
		&metrics.GaugeOpts{
			...
			Help:           "Timestamp of the last successful or failed automatic reload of authorization configuration split by status and apiserver identity.",
			StabilityLevel: metrics.ALPHA,
		},
		[]string{"status", "apiserver_id_hash"},
	)
