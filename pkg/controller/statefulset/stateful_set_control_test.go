...
func TestStatefulSetControlRollingUpdateWithMaxUnavailable(t *testing.T) {
	...
	testCases := []apps.PodManagementPolicyType{
		apps.OrderedReadyPodManagement,
		apps.ParallelPodManagement,
	}
	for _, tc := range testCases {
		...
		set := setupPodManagementPolicy(tc, newStatefulSet(totalPods))
...
func setupForInvariant(t *testing.T) (*apps.StatefulSet, *fakeObjectManager, StatefulSetControlInterface, intstr.IntOrString) {
	...
	return set, spc, ssc, maxUnavailable
}
...
func TestStatefulSetControlRollingUpdateWithMaxUnavailableInOrderedModeVerifyInvariant(t *testing.T) {
	...
	testCases := []struct {
		...
		{[]int{4, 3}},
		{[]int{5, 4, 3}},
		//	{2}, // 2 will not be terminated because its less than partition
	}
	for _, tc := range testCases {
		defer featuregatetesting.SetFeatureGateDuringTest(t, utilfeature.DefaultFeatureGate, features.MaxUnavailableStatefulSet, true)()
		set, spc, ssc, maxUnavailable := setupForInvariant(t)
		t.Run(fmt.Sprintf("terminating pod at ordinal %d", tc.ordinalOfPodToTerminate), func(t *testing.T) {
			status := apps.StatefulSetStatus{Replicas: 6}
			...
			fmt.Println("pods already terminating ", len(tc.ordinalOfPodToTerminate))
			fmt.Println("pods to update ", maxUnavailable.IntValue())
			...
			fmt.Println("pods to be deleted", expecteddPodsToBeDeleted)

			expectedPodsAfterUpdate := 6 - expecteddPodsToBeDeleted
