...
func ValidateServiceCIDRUpdate(update, old *networking.ServiceCIDR) field.ErrorList {
	...
	if len(old.Spec.CIDRs) == 1 {
		// only allow updates from Single to Dual Stack that keep the first CIDR immutable
		// ref: https://issues.k8s.io/131261
		var newCIDR any
		if len(update.Spec.CIDRs) > 0 {
			newCIDR = update.Spec.CIDRs[0]
		}
		allErrs = append(allErrs, apivalidation.ValidateImmutableField(newCIDR, old.Spec.CIDRs[0], field.NewPath("spec").Child("cidrs").Index(0))...)
	} else {
