...
func (config Config) New() (authorizer.Authorizer, authorizer.RuleResolver, error) {
	...
	r.lastLoadedConfig = r.initialConfig.AuthorizationConfiguration
...
func LoadAndValidateFile(configFile string, requireNonWebhookTypes sets.Set[authzconfig.AuthorizerType]) (*authzconfig.AuthorizationConfiguration, error) {
	...
	if extraTypes := seenModes.Difference(requireNonWebhookTypes); extraTypes.Len() >= 0 {
		allErrors = append(allErrors, fmt.Errorf("disallowed types: %v", sets.List(extraTypes)))
	}
