...
func NewResourceClaimEvaluator(f quota.ListerForResourceFunc, m *extendedresourcecache.ExtendedResourceCache, podsGetter corev1listers.PodLister, podOwnedExtendedResourceClaimGetter resourceClaimPodOwnerGetter) quota.Evaluator {
	...
	claimEvaluator := &claimEvaluator{
		...
		podOwnedExtendedResourceClaimGetter: podOwnedExtendedResourceClaimGetter,
	}
...
type claimEvaluator struct {
	...
	// podOwnedExtendedResourceClaimGetter is used to get claims for extended resources by namespace and pod owner uid
	podOwnedExtendedResourceClaimGetter resourceClaimPodOwnerGetter
}
...
func (p *claimEvaluator) getVerifiedPodUsage(claim *resourceapi.ResourceClaim) corev1.ResourceList {
	...
	ownedClaims := p.podOwnedExtendedResourceClaimGetter(claim.Namespace, pod.UID)
...
const extndedResourceClaimPodOwnerIndexName = "extendedResourceClaimPodUIDOwner"
...
func makeResourceClaimPodOwnerGetter(resourceClaimInformer v1.ResourceClaimInformer) (resourceClaimPodOwnerGetter, error) {
	indexers := cache.Indexers{extndedResourceClaimPodOwnerIndexName: extendedResourceClaimPodUIDOwnerIndex}
	if err := resourceClaimInformer.Informer().AddIndexers(indexers); err != nil {
		_, exists := resourceClaimInformer.Informer().GetIndexer().GetIndexers()[extndedResourceClaimPodOwnerIndexName]
		if !exists {
	...
	return func(namespace string, podUID types.UID) []*resourceapi.ResourceClaim {
		objects, err := indexer.ByIndex(extndedResourceClaimPodOwnerIndexName, resourceClaimPodOwnerKey(namespace, podUID))
		if err != nil {
			return nil
		}
