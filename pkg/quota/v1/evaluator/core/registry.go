...
func NewEvaluators(f quota.ListerForResourceFunc, i informers.SharedInformerFactory) ([]quota.Evaluator, error) {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.DynamicResourceAllocation) {
		...
		var resourceClaimPodOwnerGetter resourceClaimPodOwnerGetter
		if utilfeature.DefaultFeatureGate.Enabled(features.DRAExtendedResource) {
			...
			var err error
			resourceClaimPodOwnerGetter, err = makeResourceClaimPodOwnerGetter(i.Resource().V1().ResourceClaims())
			if err != nil {
				return nil, err
			}
		}
		result = append(result, NewResourceClaimEvaluator(f, deviceClassMapping, podLister, resourceClaimPodOwnerGetter))
	}
