...
var (
	lastKeyFetchTimeStamp = metrics.NewGaugeVec(
		&metrics.GaugeOpts{
			Namespace:      namespace,
			Subsystem:      subsystem,
			Name:           "supported_keys_last_fetch_time",
			Help:           "Timestamp of the last successful key sync",
	...
	dataTimeStamp = metrics.NewGaugeVec(
		&metrics.GaugeOpts{
			Namespace:      namespace,
			Subsystem:      subsystem,
			Name:           "supported_keys_data_timestamp",
			Help:           "Unix Timestamp in seconds associated with snapshot of supported keys as returned by external signer",
			StabilityLevel: metrics.ALPHA,
		},
		nil,
	)
	...
	outboundRequest = metrics.NewHistogramVec(
...
func RegisterMetrics() {
	registerMetrics.Do(func() {
		...
		legacyregistry.MustRegister(outboundRequest)
	})
}
...
func OuboundRequestMetricsInterceptor(ctx context.Context, method string, req, reply any, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
	...
	outboundRequest.WithLabelValues(method, status.Code(err).String()).Observe(time.Since(start).Seconds())
	return err
}
