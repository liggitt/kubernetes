...
func BuildAuth(nodeName types.NodeName, client clientset.Interface, config kubeletconfig.KubeletConfiguration) (server.AuthInterface, func(<-chan struct{}), error) {
	...
	authenticator, runAuthenticatorCAReload, clientCAContent, err := BuildAuthn(tokenClient, config.Authentication)
	if err != nil {
		return nil, nil, err
	}
	...
	return server.NewKubeletAuth(authenticator, attributes, authorizer, clientCAContent), runAuthenticatorCAReload, nil
}
