...
func (rc *ResourceConsumer) GetReplicas() int {
	...
	case KindCRD:
		...
		deploymentReplicas := int(deployment.Status.ReadyReplicas)
		...
		framework.ExpectNoError(unstructured.SetNestedField(crdInstance.Object, fmt.Sprint(deploymentReplicas), "status", "replicas"))
...
func (rc *ResourceConsumer) CleanUp() {
	...
	if kind.Kind == crdKind {
		...
		if err != nil && !apierrors.IsNotFound(err) {
			framework.Failf("Error while deleting CRD of kind %v: %v", kind, err)
		}
