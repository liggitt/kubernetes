...
func TestCustomResourceValidatorsWithBlockingErrors(t *testing.T) {
	...
	t.Run("Structural schema", func(t *testing.T) {
		...
		t.Run("custom resource create and update MUST NOT allow data if there is blocking error of MaxLength", func(t *testing.T) {
			...
			_, err := crClient.Create(context.TODO(), cr, metav1.CreateOptions{})
			if err == nil || !strings.Contains(err.Error(), "Skipping x-kubernetes-validations rules due to OpenAPIv3 errors preventing the rules from safely executed") {
				t.Fatalf("expect error to contain \"Skipping x-kubernetes-validations rules due to OpenAPIv3 errors preventing the rules from safely executed\" but get: %v", err)
			}
		})
