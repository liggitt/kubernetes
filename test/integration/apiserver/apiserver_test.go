...
func TestTransformOnWatch(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name:   "verify columns on CRDs",
			accept: "application/json;as=Table;g=meta.k8s.io;v=v1beta1",
			object: func(t *testing.T) (metav1.Object, string, string) {
				cr, err := crclient.Create(&unstructured.Unstructured{map[string]interface{}{"apiVersion": "cr.bar.com/v1", "kind": "Foo", "metadata": map[string]interface{}{"name": "test-1"}}}, metav1.CreateOptions{})
				if err != nil {
		...
		},
		{
			...
			// TODO: uncomment when https://github.com/kubernetes/kubernetes/pull/73937 merges
