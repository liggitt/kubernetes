...
func (r envelopekmsv2) cipherTextDEK() ([]byte, error) {
	prefixLen := len(encryptedProtoEncodingPrefix)
	payload := r.rawEnvelope[prefixLen:]

	o := &kmstypes.EncryptedObject{}
	if err := o.Unmarshal(payload[r.startOfPayload(r.providerName):]); err != nil {
		return nil, err
	}
	return o.EncryptedDEK, nil
}

func (r envelopekmsv2) startOfPayload(providerName string) int {
	return r.prefixLen()
}
...
// 1. Raw records in ETCD that were processed by KMSv2 Provider should be prefixed with k8s:enc:kms:v2:grpc-kms-provider-name:
...
`
	...
	wantPrefix := "k8s:enc:kms:v2:kms-provider:"
